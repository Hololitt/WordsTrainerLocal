...
public String defineTrainingType(){
    ...
    LanguageCardsWritingEnabled languageCardsWritingEnabled =
            userSettingsService.getUserSettings(userService.getUserId()).getLanguageCardsWritingEnabled();
...
public ResponseEntity<List<LanguageCard>> showLanguageCardsLearningPage(){
    ...
    private TrainingContextDTO createTrainingDTO(LanguageCard card) {
        return TrainingContextDTO.builder()
                ...
                .correctAnswersCountToFinish(userSettingsService.getUserSettings(userService.getUserId())
    ...
    public String checkAnswer(@ModelAttribute("answer") TranslationModel translationModel, Model model) {
        ...
        FlashCardsTrainingVariety flashTrainingType = userSettingsService.getUserSettings(userService.getUserId())
        ...
        if (!result.isCorrect()) {
            model.addAttribute("userAnswer", translationModel.getAnswer());
    ...
    public String finish(Model model) {
        ...
        lastLearnedLanguageCardsService.deleteStatusLastLearnedCard((int) userService.getUserId());
        ...
        languageCardService.saveLanguageCardList(languageCards, userService.getUserId());
    ...
    public String repeatLastLearned() {
        ...
        List<LanguageCard> lastLearnedLanguageCards = lastLearnedLanguageCardsService.
                findLastLearnedLanguageCards(userId);

        return repeatLanguageCardsOrReturnException(lastLearnedLanguageCards);
    }

    private String repeatLanguageCardsOrReturnException(List<LanguageCard> languageCards) {
        long userId = userService.getUserId();
        FlashCardsTrainingVariety flashCardsTrainingVariety = userSettingsService.getUserSettings(userService.getUserId())
                .getFlashCardsTrainingVariety();

        if (languageCardValidator.isLanguageCardListValid(languageCards)) {
            languageCardContextHolder.cleanUpContext();
            languageCardsTrim(languageCards);
            languageCardContextHolder.setLanguageCardsToLearn(languageCards);
            languageCardContextHolder.setContext();
            return defineFlashCardsTrainingType(flashCardsTrainingVariety);
        }
        return "emptyListException";
    }

    private void languageCardsTrim(List<LanguageCard> languageCards){
        languageCards.forEach(lc -> {
            lc.setWord(lc.getWord().trim());
            lc.setTranslation(lc.getTranslation().trim());
        });
    }
